...
export const getNodeQty = async ({
  ...
  try {
    ...
    if (isSupportBsc) {
      ...
      peersETH = results[0].length;
      peersBSC = results[1].length;
    } else {
      const result = await fetcher<Array<any>>(`${urlEth}/api/v1/peers`);
      peersETH = result.length;
    }
    return peersETH + peersBSC;
  } catch (e) {
    return 0;
  }
};
